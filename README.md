...
The CLI exports song JSON and waits for your model response JSON input:
...
- provide model JSON by piping/pasting as input
...
- `ytmo stats` shows friendly plan checks (`Ready`, `Needs plan file`, `Needs liked songs snapshot`, `Plan file needs fixes`, `Liked songs file needs fixes`) and warnings instead of failing on missing/corrupt local artifacts.
...
- Dependabot keeps Python and GitHub Actions dependencies updated automatically
