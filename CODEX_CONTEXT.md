...
- Interactive auth setup defaults to browser-assisted capture via `ytmo setup --auth-method auto`: it opens YouTube Music in a workspace-owned Playwright profile, gives explicit login/terminal instructions, tries to bring the browser page forward, captures authenticated `/youtubei/v1/browse` request headers, writes `browser.json`, and falls back to manual paste when capture is unavailable. If Playwright Chromium is missing, setup asks before installing it with the current Python runtime and retries capture once. `--auth-method browser` requires capture; `--auth-method manual` uses paste-only setup.
- Interactive `ytmo sync` can repair a missing configured auth file after `bootstrap.json` confirms setup completed, using the same browser-first auth flow before continuing. This is auth repair only: first-time/incomplete workspaces still require `ytmo setup`, and `--non-interactive`/`--dry-run` sync still require an existing readable auth file.
...
- Human-facing CLI output uses a TTY-first renderer with guided stepper progress for setup/sync/rebuild/demo, shared wait/status indicators for longer network/browser/playlist/classification work, recap cards for command completion, and callout-style confirmations.
