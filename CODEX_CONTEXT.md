...
- CLI parse/usage errors now print scoped help with exit code `2`: top-level parse failures show top-level help, while subcommand parse failures (for example `ytmo setup -q`) show that subcommand’s help. Implementation uses a custom `ArgumentParser.error()` override to raise `argparse.ArgumentError`, ensuring consistent behavior across Python `3.11`/`3.12`/`3.13` where `exit_on_error=False` alone can still exit early.
